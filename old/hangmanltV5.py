...
class HangmanGUI: # Defining class
	...
	def check_word(self, guess): # Method to check the player's input
								 # Take the player's input as a parameter, so that I don't have to type out a self. prefix everytime
								 # I want to use their input
		# print(self.guess.get()) # Here for debug purposes
		if self.declare_message.get() == "": # Check if the game is still in progress
			...
			self.guess.set("") # Reset entry field
			for letter in self.random_word:
				print(letter)
	...
	def draw_gui(self): # Method to generate the GUI
		...
		self.guess_button = ttk.Button(self.main_frame, text="Guess", command=lambda : self.check_word(self.guess)) 
