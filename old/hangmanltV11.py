...
contingency_words_list = ["hanging", "man", "never", "liar", "fantastic", "not", "chance", "always", "your", "program", "smite", "tank",
...
multiple_letters_list = ["happy"] # Debug list to test words that have the same letter occ
...
class HangmanGame: # Defining class
	def __init__(self, master): # Constructor class, always uses root as argument
		...
		self.master = master # Define as the parameter
	...
	def generate_word(self): # Method for resetting the entire game
		...
		print(self.random_word) # Debug message
	...
	def check_word(self, guess): # Method to check the player's input
		...
		guess = guess.lower() # Make the player's input lowercase to broaden the number of accepted inputs
		self.initial_hangman_count = self.hangman_count # How many lines that were drawn at the beginning of the check
		...
		if self.declare_message.get() == "": # Check if the game is still in progress
			...
			if guess.isalpha() == True: # Check if the player's input only uses letters
				...
				if guess == self.random_word: # If the player guesses the whole word
					...
					self.correct = "nil" # Set this variable to a hardcoded string, where the player isn't penalized 
				...
				if self.correct == True: # Player guesses correctly
					...
					self.status_message.configure(foreground=self.right_color)
				elif self.correct == "nil": # Player attempts to guess a word, but that word isn't long enough
					...
					self.status_message.configure(foreground=self.wrong_color)
			...
			if self.hangman_count != self.initial_hangman_count: # Check if the hangman count value is the same as the initial count value 
				if self.hangman_count == 6: # Seventh value in the list is the hangman's head
