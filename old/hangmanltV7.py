...
continegency_words_list = ["hanging", "man", "never", "liar", "fantastic", "amazing", "not", "chance", "always", "your", "program", "smite", "tank",
							...
							"memes", "end", "here", "rules", "of", "nature"] 
...
class HangmanGUI: # Defining class
	def __init__(self, master): # Constructor class, always uses root as argument
		...
		self.guess_count = IntVar()
		self.guess_count.set(10)
	...
	def generate_word(self): # Method for generating a random word
		...
		self.random_word = random.choice(multiple_letters_list) # Random word is defined within the method so a parameter is not needed
				...
				if guess == self.random_word: # If the player guesses the whole word
					...
					for i in range(0, len(self.random_word)):
						if guess == self.random_word[i]:
							...
							found_letter = True
							if self.hidden_word.get() == self.random_word:
								self.declare_message.set("You win")

					if not found_letter:
							self.guess_count.set(self.guess_count.get()-1)
							self.guess_letters_list.append(guess.lower())
							...
							if self.guess_count.get() <= 0:
								self.declare_message.set("You lose")
